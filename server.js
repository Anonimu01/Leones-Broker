...
function normalizeQty(body = {}) {
  const n = Number(
    body.qty ?? body.quantity ?? body.amount ?? body.positionSize ?? body.notional ?? body.size
  );
...
function annotatePosition(position = {}) {
  const currentPrice =
    toNumber(
      position.currentPrice ?? getCurrentPriceForSymbol(position.symbol) ?? position.price ?? position.entryPrice ?? 0
    ) ?? 0;
...
function emitStateUpdates(userId, accountPayload = null, positions = null, transaction = null) {
  try {
    ...
    io.emit("wallet_update", { userId, account });
    io.emit("account_update", { userId, account });
...
async function buildAccountForUser(user) {
  ...
  const balance = toNumber(wallet?.balanceOwn ?? wallet?.balance ?? user.balance ?? 0) ?? 0;
...
app.get("/api/market/latest", (req, res) => res.json(buildMarketPayload()));
...
app.get("/api/market/polygon/latest", (req, res) => res.json(buildMarketPayload()));
app.get("/api/market/polygon/symbols", (req, res) => res.json(buildMarketPayload().symbols));
...
/* ======================================================
   ROUTE MOUNTS (compatibility)
   ====================================================== */
try { if (authRoutes) app.use("/api/auth", authRoutes); } catch (e) { console.warn("authRoutes mount error:", e?.message || e); }
try { if (userRoutes) app.use("/api/users", userRoutes); } catch (e) { console.warn("userRoutes mount error:", e?.message || e); }
try { if (verificationRoutes) app.use("/api/verification", verificationRoutes); } catch (e) { console.warn("verificationRoutes mount error:", e?.message || e); }
try { if (walletRoutes) app.use("/api/wallet", walletRoutes); } catch (e) { console.warn("walletRoutes mount error:", e?.message || e); }
try { if (positionsRoutes) app.use("/api/positions", positionsRoutes); } catch (e) { console.warn("positionsRoutes mount error:", e?.message || e); }
try { if (accountRoutes) app.use("/api/account", accountRoutes); } catch (e) { console.warn("accountRoutes mount error:", e?.message || e); }
