...
function normalizeUniversalSymbol(symbol = "") {
  ...
  if (parts.length > 1) {
    ...
    if (/^(\d+|[A-Z]\d*|\d+[A-Z]*)$/i.test(last)) s = prev || last;
    else s = last;
  }
...
function getMarketHint(symbol = "") {
  ...
  if (clean === "GOLD" || clean === "SILVER" || clean === "OIL" || clean === "BRENT" || clean.includes("NATGAS") || clean.includes("COPPER") || clean.includes("CORN") || clean.includes("WHEAT") || clean.includes("SOYBEAN") || clean.includes("PLATINUM") || clean.includes("PALLADIUM")) return "commodity";
  if (clean.includes("BTC") || clean.includes("ETH") || clean.includes("SOL") || clean.includes("BNB") || clean.includes("ADA") || clean.includes("XRP") || clean.includes("DOGE") || clean.includes("LTC") || clean.includes("LINK") || clean.includes("AVAX") || clean.includes("MATIC") || clean.includes("USDT") || clean.includes("USDC") || clean.includes("BUSD")) return "crypto";
...
function findBestPriceMatch(symbol, store = {}) {
  ...
  for (const key of keys) {
    ...
    const candidates = [key, item?.symbol, item?.ticker, item?.sym, item?.tvSymbol, item?.instrument, item?.marketSymbol, item?.asset, item?.name, item?.label].filter(Boolean);
...
function normalizePositionSymbol(body = {}) {
  const raw = String(body.symbol || body.tvSymbol || body.selectedSymbol || body.chartSymbol || body.instrument || body.marketSymbol || body.market || body.ticker || body.asset || "").trim().toUpperCase();
...
function formatSymbolLabel(symbol = "") {
  ...
  if (/^[A-Z]{6}$/.test(s) && getMarketHint(s) === "forex") return `${s.slice(0, 3)}/${s.slice(3)}`;
  return s;
}
...
function extractQuotePrice(item = {}, depth = 0) {
  ...
  const direct = toNumber(item.price) ?? toNumber(item.last) ?? toNumber(item.close) ?? toNumber(item.value) ?? toNumber(item.mark) ?? toNumber(item.mid) ?? toNumber(item.c) ?? toNumber(item.p) ?? toNumber(item.lp);
  ...
  const nestedCandidates = [item.results, item.result, item.data, item.trade, item.quote, item.lastTrade, item.t, item.q, item.raw, item.items, item.payload, item.body].filter(Boolean);
...
function detectMarket(symbol = "") {
  ...
  if (s === "GOLD" || s === "SILVER" || s === "OIL" || s === "BRENT" || s.includes("NATGAS") || s.includes("COPPER") || s.includes("CORN") || s.includes("WHEAT") || s.includes("SOYBEAN") || s.includes("PLATINUM") || s.includes("PALLADIUM")) return "commodities";
...
function getVolatility(symbol = "") {
  ...
  switch (market) {
    case "crypto": return s.includes("BTC") ? 0.008 : s.includes("ETH") ? 0.010 : 0.012;
    case "forex": return s.includes("JPY") ? 0.0018 : 0.0012;
    case "indices": return s.includes("VIX") ? 0.02 : 0.004;
    case "commodities": return s.includes("OIL") || s.includes("WTI") || s.includes("BRENT") ? 0.008 : 0.005;
    case "stocks":
...
function writePrice(symbol, price, raw = null) {
  ...
  const payload = { symbol: clean, price: numeric, market: detectMarket(clean), updatedAt: new Date().toISOString(), raw };
  global.priceCache[clean] = numeric;
  global.priceMeta[clean] = payload;
  try {
    if (priceHandler?.prices instanceof Map) priceHandler.prices.set(clean, payload);
    else if (priceHandler?.prices && typeof priceHandler.prices === "object") priceHandler.prices[clean] = payload;
  } catch {}
  return true;
}
...
function seedInitialSymbols() {
  for (const group of Object.values(MARKET_SEEDS)) {
    for (const [sym, px] of Object.entries(group)) writePrice(sym, px, { source: "seed" });
  }
}
...
function safeStorePrice(symbol, price, rawData = null) {
  ...
  const numeric = Number(price);
  if (!clean || !Number.isFinite(numeric) || numeric <= 0) return false;
  const payload = { symbol: clean, price: numeric, market: detectMarket(clean), updatedAt: new Date().toISOString(), raw: rawData || null };
  global.priceCache[clean] = numeric;
  global.priceMeta[clean] = payload;
  try {
    if (priceHandler?.prices instanceof Map) priceHandler.prices.set(clean, payload);
...
async function fetchPolygonLastPrice(symbol) {
  try {
    ...
    if (price && Number.isFinite(price) && price > 0) return { price, raw: data, symbol: clean, polygonSymbol };
    return null;
  } catch {
    return null;
  }
...
async function resolveUniversalPrice(rawSymbol) {
  try {
    ...
    if (!Number.isFinite(price) || price <= 0) price = Number(global.priceCache?.[symbol]);
    if (!Number.isFinite(price) || price <= 0) price = global.getFakePrice(symbol);
    if (!Number.isFinite(price) || price <= 0) {
    ...
    if (!Number.isFinite(price) || price <= 0) price = getRealisticFallbackPrice(symbol);
...
async function getLatestPriceResponse(rawSymbol) {
  ...
  return { ok: true, symbol, price, currentPrice: price, last: price, close: price, updatedAt: new Date().toISOString() };
}
...
async function getWalletDocForUser(userId) {
  ...
  if (!wallet) {
    wallet = new Wallet({ user: userId, balanceOwn: 0, balance: 0, credit: 0, marginUsed: 0, leverageFactor: 1, equity: 0, freeMargin: 0, marginLevel: 0, currency: "USD" });
  }
  return wallet;
}
...
function normalizeWalletSnapshot(wallet, openPnl = 0) {
  ...
  return { balance: balanceOwn, balanceOwn, credit, equity, marginUsed, freeMargin, availableBalance: freeMargin, marginLevel, leverageFactor: Number(wallet?.leverageFactor ?? 1) || 1, currency: "USD", openPnl: pnl };
}
...
async function buildAccountForUser(userDoc) {
  ...
  return { account: { ...normalizedWallet, balance, balanceOwn: balance, availableBalance: normalizedWallet.availableBalance, equity: normalizedWallet.equity, leverage: Number(userDoc.leverage ?? walletSnapshot?.leverageFactor ?? 100) || 100, currency: "USD", positions: openPositions, openPositions, allPositions, recentTransactions, transactions: recentTransactions, openPnl }, user: userDoc.toObject ? userDoc.toObject() : userDoc, wallet: walletSnapshot, positions: openPositions, transactions: recentTransactions };
}
...
async function requireAdmin(req, res, next) {
  try {
    ...
    if (role === "admin" || payload?.isAdmin === true || payload?.admin === true) { req.user = payload; return next(); }
...
function buildEmptyAccountPayload(userDoc) {
  const walletSnapshot = normalizeWalletSnapshot({ balanceOwn: 0, balance: 0, credit: 0, marginUsed: 0, leverageFactor: Number(userDoc?.leverage ?? 1) || 1, equity: 0, freeMargin: 0, marginLevel: 0, currency: "USD" }, 0);
  return { account: { ...walletSnapshot, balance: 0, balanceOwn: 0, availableBalance: 0, equity: 0, leverage: Number(userDoc?.leverage ?? 100) || 100, currency: "USD", positions: [], openPositions: [], allPositions: [], recentTransactions: [], transactions: [], openPnl: 0 }, user: userDoc?.toObject ? userDoc.toObject() : userDoc || null, wallet: walletSnapshot, positions: [], transactions: [] };
}

async function safeBuildAccountForUser(userDoc) { try { return await buildAccountForUser(userDoc); } catch (err) { console.error("buildAccountForUser failed:", err); return buildEmptyAccountPayload(userDoc); } }
async function safeLoadOpenPositionsForUser(userId) { try { const rows = await Position.find({ user: userId, status: { $in: ["OPEN", "open", "Open"] } }).sort({ createdAt: -1 }).lean().exec().catch(() => []); return (rows || []).map((p) => { try { return annotatePosition(p); } catch { return p; } }); } catch (err) { console.error("safeLoadOpenPositionsForUser failed:", err); return []; } }
async function safeLoadAllPositionsForUser(userId) { try { const rows = await Position.find({ user: userId }).sort({ createdAt: -1 }).lean().exec().catch(() => []); return (rows || []).map((p) => { try { return annotatePosition(p); } catch { return p; } }); } catch (err) { console.error("safeLoadAllPositionsForUser failed:", err); return []; } }
async function safeLoadTransactionsForUser(userId, limit = 50) { try { return await loadTransactionsForUser(userId, limit); } catch (err) { console.error("safeLoadTransactionsForUser failed:", err); return []; } }
async function safeGetUserFromBearer(req) { try { return await getUserDocFromBearer(req); } catch (err) { console.error("getUserDocFromBearer failed:", err); return null; } }
...
async function applyCloseToPosition({ user, positionDoc, currentPrice, source = "api/trade/close" }) {
  ...
  const tx = await recordTransaction({ user, type: "trade_close", amount: realizedPnl, status: "completed", note: `${side} ${symbol}`, balanceBefore, balanceAfter: wallet.balanceOwn, meta: { positionId: String(position._id), symbol, side, qty, entryPrice, closePrice: closePx, marginReleased: reservedMargin, realizedPnl }, source });
  const account = await safeBuildAccountForUser(user);
  const annotatedPosition = annotatePosition(position);
  emitStateUpdates(user._id, account, [annotatedPosition], tx);
  return { positionId: position._id, symbol, side, qty, entryPrice, currentPrice: closePx, realizedPnl, balance: wallet.balanceOwn, account: account.account, wallet: account.wallet, position: annotatedPosition, transaction: tx };
}
...
async function buildQuotesArray() {
  ...
  const extraKeys = Object.keys(getPriceStore()).filter((key) => !SAMPLE_SYMBOLS.some((sample) => buildSymbolAliases(sample.symbol).some((alias) => buildSymbolAliases(key).includes(alias))));
...
async function buildMarketPayload() {
  ...
  return { ok: true, count: quotes.length, quotes, data: quotes, items: quotes, latest };
}
...
app.post("/api/trade/open", async (req, res) => {
  ...
  try {
    ...
    if (!withOpenLock(lockKey, 2500) || !withActiveOrder(lockKey, 2500)) return res.status(429).json({ ok: false, error: "duplicate_order_blocked" });
    const wallet = await getWalletDocForUser(user._id);
    ...
    if (!Number.isFinite(price) || price <= 0) price = global.getFakePrice?.(symbol) || forcePriceExists?.(symbol) || getRealisticFallbackPrice(symbol) || 100;
    price = Number(price);
    if (!Number.isFinite(price) || price <= 0) price = 100;
    const notional = qty * price;
    const requiredMargin = notional / leverage;
    const freeMargin = balanceOwn + credit - marginUsed;
    if (freeMargin < requiredMargin) return res.status(400).json({ ok: false, error: "insufficient_margin", requiredMargin, freeMargin });
    ...
    wallet.updatedAt = new Date();
    await wallet.save();
    const position = await Position.create({ user: user._id, symbol, side, qty, entryPrice: price, currentPrice: price, marginReserved: requiredMargin, leverage, status: "OPEN", createdAt: new Date(), updatedAt: new Date() });
    const account = await safeBuildAccountForUser(user);
    ...
    return res.json({ ok: true, msg: "OPENED", position, wallet: account.wallet, account: account.account, symbol, entryPrice: price });
  } catch (err) {
    console.error("/api/trade/open error:", err);
    return res.status(500).json({ ok: false, error: "server_error", message: err?.message || "Error interno" });
  } finally {
    if (lockKey) { releaseOpenLock(lockKey); releaseActiveOrder(lockKey); }
  }
});
...
app.post("/api/trade/close", async (req, res) => {
  ...
  try {
    ...
    if (!Number.isFinite(price) || price <= 0) price = global.getFakePrice?.(position.symbol) || forcePriceExists?.(position.symbol) || getRealisticFallbackPrice(position.symbol) || position.entryPrice;
    price = Number(price);
...
app.use(async (req, res, next) => {
  ...
  try {
    ...
    if (candidate) {
      ...
      return res.status(200).type("application/javascript; charset=utf-8").send(cleaned);
    }
    return res.status(404).type("application/javascript; charset=utf-8").send(`console.error("JS missing: ${pathname}");`);
  } catch (err) {
    console.error("Error sirviendo JS:", err);
    return res.status(500).type("application/javascript; charset=utf-8").send(`console.error("JS server error");`);
  }
});
...
app.get("/dashboard.html", (req, res) => {
  ...
  if (!fs.existsSync(filePath)) return res.status(404).send("dashboard.html no encontrado");
...
app.get("/", (req, res) => {
  ...
  if (!fs.existsSync(filePath)) return res.status(404).send("index.html no encontrado");
  return res.sendFile(filePath);
});
...
app.get("/api/price", async (req, res) => {
  try {
    ...
    return res.json({ ok: true, symbol: latest.symbol, price: Number(latest.price), currentPrice: Number(latest.price), last: Number(latest.price), close: Number(latest.price), updatedAt: latest.updatedAt || new Date().toISOString() });
  } catch (err) {
    try {
      ...
      if (fallbackSymbol) {
        ...
        return res.json({ ok: true, symbol: fallbackSymbol, price: fallbackPrice, currentPrice: fallbackPrice, last: fallbackPrice, close: fallbackPrice, updatedAt: new Date().toISOString() });
      }
    } catch {}
